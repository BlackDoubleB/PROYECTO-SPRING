...
public class ReservaService {
	...
	public Reserva create(ReservaCreateRequest request) {
		...
		if(ultimaReserva != null) {
			...
			ticket = String.format("A%05d", t + 1);
		}
	...
	public List<Reserva> findAllToDataTable(Integer idConsumidor, String fecha) throws ParseException{
		
		if(fecha.isEmpty()) {
			
			return reservaRepository.findAllByConsumidorIdConsumidor(idConsumidor);
		...
		return reservaRepository.findAllByConsumidorIdConsumidorAndHorarioReservaFecha(idConsumidor, sqlDate);
		
	}
	
	public List<Reserva> findAllByConsumidorIdConsumidor(Integer idConsumidor){
